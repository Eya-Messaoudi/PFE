...
const TeacherTextBox = ({ targetId, idP }) => {
  ...
  useEffect(() => {
    const fetchDiscussion = async () => {
      const response = await fetch(API_BASE + "/discussion/" + idP, {
        headers: {
  ...
  const sendMessage = async () => {
    const response = await fetch(API_BASE + "/sendMessage/" + idP, {
      method: "POST",
      headers: {
                  ...
                  {!(message.sender === idP) && (
                    <div className="row g-1 mt-3 text-white">
                      <div className="col-8 text-end">
                        <p
                          className="p-3 bg bg-primary rounded"
                          style={{ display: "inline-block", width: "auto" }}
                        >
                          {message.contenu}
                        </p>
